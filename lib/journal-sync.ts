...
import { feedJournalToAvatar } from '@/lib/avatar-feed';
...
async function triggerJournalEmbedding(journalId: string): Promise<void> {
  // #region agent log
  fetch('http://127.0.0.1:7744/ingest/3aeff884-f869-49e5-b3ca-024823977ed4',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'d7c109'},body:JSON.stringify({sessionId:'d7c109',runId:'upload-train-1',hypothesisId:'C',location:'lib/journal-sync.ts:triggerJournalEmbedding:start',message:'Triggering journal embedding',data:{journalId},timestamp:Date.now()})}).catch(()=>{});
  // #endregion
  await feedJournalToAvatar(journalId);
  // #region agent log
  fetch('http://127.0.0.1:7744/ingest/3aeff884-f869-49e5-b3ca-024823977ed4',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'d7c109'},body:JSON.stringify({sessionId:'d7c109',runId:'upload-train-1',hypothesisId:'C',location:'lib/journal-sync.ts:triggerJournalEmbedding:done',message:'Journal embedding trigger finished',data:{journalId},timestamp:Date.now()})}).catch(()=>{});
  // #endregion
}
